...
void modificarCandidato(void) {
	...
	char escolha = 'x';
	infoCandidato candidato,  candidatos[nroRegistros];
	...
	if(arquivoCandidato == NULL) { // fix ***************** nessa parte falta verificar se tam arq == 0 ***************************
		...
		identifCandidato = validaEscolhaLegenda(&nroLegendaCand, arquivoCandidato);
		
		fread(&candidatos, sizeof(candidato), nroRegistros, arquivoCandidato);
		//
		...
		while (escolha != '0') {
			...
			moverCursor(POSMENU, ++aux);
			printf("2. número de legenda:     %d",candidato.numeroLegenda);
			moverCursor(POSMENU, ++aux);
			...
			switch(escolha) {
				...
				case '0':
					...
					fwrite(&candidatos, sizeof(candidato), nroRegistros, arquivoCandidato);
					fecharArquivo(arquivoCandidato);
